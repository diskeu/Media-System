...
class VoteRepo(BaseRepo):
    ...
    async def get_users_vote(self, user_id, *post_ids: str) -> dict[int, int] | BaseRepo.RepoError:
        """
        ...
        """
        sql_return_val: list = await self.get_all_enriched(
    ...
    async def vote(self, user_id, post_id, mode: int) -> None | BaseRepo.RepoError:
        """
        ...
        if mode == 3:
            ...
            return await self.execute_write(delete_query, user_id, post_id) # None | RepoError
        ...
        return await self.execute_write(insert_query, *insert_vals) # None | RepoError